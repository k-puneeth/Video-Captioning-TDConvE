from data_loader import you_cook2_dataset
...
def train(args):
	...
	#training vocab_file,dataset_dir,meta_file,batch,max_len
	train_meta=you_cook2_dataset("./vocab.txt",args.train_vocab,"./youcookii_annotations_trainval.json",args.batch_size, 237)
	train=DataLoader(train_meta,batch_size=1,shuffle=False)

	#BLEU evaluation
	BLEU_train_meta=you_cook2_dataset("./vocab.txt",args.train_vocab,"./youcookii_annotations_trainval.json",1, 237)
	BLEU_test_meta=you_cook2_dataset("./vocab.txt",args.test_vocab,"./youcookii_annotations_trainval.json",1, 237)
	...
	for i_epoch in range(start,args.epoch):
		for i_b,(images,sen_in,lengths) in enumerate(train):
			...
			# print(outputs.reshape(-1,outputs.shape[2]).shape, sen_in[:,1:].reshape(-1).shape)
		...
		in_BLEU=0
		out_BLEU=0
		for i_b,(images,sens,_) in enumerate(BLEU_train):
			#print(i_b,"/",int(len(BLEU_train)*args.BLEU_eval_ratio))
			if i_b==int(len(BLEU_train)*args.BLEU_eval_ratio):
				break
			images=images.squeeze(0).to(device)
			sens.squeeze(0).to(device)
			in_BLEU+=get_BLEU(images,encoder,decoder,train_meta,BLEU_train_meta,sens)

		for i_b,(images,sens,_) in enumerate(BLEU_test):
			#print(i_b,"/",int(len(BLEU_test)*args.BLEU_eval_ratio))
			if i_b==int(len(BLEU_test)*args.BLEU_eval_ratio):
				break
			images=images.squeeze(0).to(device)
			sens.squeeze(0).to(device)
			out_BLEU+=get_BLEU(images,encoder,decoder,train_meta,BLEU_test_meta,sens)
...
def get_BLEU(images,encoder,decoder,train_meta,score_meta,sens):
		...
		# print(predict.shape, sens.shape)
		hypo =  train_meta.decode_sentence(predict)[0]
		orig = train_meta.decode_sentence(sens[0])[0]
		...
		return sentence_bleu(orig,hypo)
...
if __name__=="__main__":
	...
	parser.add_argument('--image_dir',default='./data/feat_dat/train_frame_feat/',help='directory for sampled images')
	parser.add_argument('--train_vocab',default='./data/feat_dat/train_frame_feat/',help='vocabulary file for training data')
	parser.add_argument('--test_vocab',default='./data/feat_dat/val_frame_feat/',help='vocabulary file for testing data')
